...
namespace SimpleServiceTests
{
    public class MockQuizServiceTests
    {
        ...
        [Fact]
        public async Task GetCompletedQuizzesAsync_ScoreCalculationIsCorrect()
        {
            // Act
            var quizzes = await _quizService.GetCompletedQuizzesAsync();
            ...
            {
                double expectedScore = quiz.QuestionsCount > 0 
                    ? (double)quiz.CorrectAnswersCount / quiz.QuestionsCount 
                    : 0;
                
                Assert.Equal(expectedScore, quiz.Score);
            }
        }
